...
{
    internal class DiscordInline : MarkdownInline
    {
        ...
        {
            tripCharHelpers.Add(new ParseHelpers.InlineTripCharHelper() { FirstChar = '<', Method = ParseHelpers.InlineParseMethod.Discord });
        }
        ...
        internal static ParseHelpers.InlineParseResult Parse(string markdown, int start, int maxEnd)
        {
            ...
            {
                return new ParseHelpers.InlineParseResult(new DiscordInline() { Id = id, DiscordType = type }, innerStart - 1, innerEnd + 1);
            }
        ...
        private static ParseHelpers.InlineParseResult HandleEmote(string text, StringBuilder builder, int index, int innerStart, int innerEnd)
        {
            ...
            if (ulong.TryParse(builder.ToString(), out id))
            {
                return new ParseHelpers.InlineParseResult(new DiscordInline() { Id = id, DiscordType = MentionType.Emote, Text = emoteName }, innerStart - 1, innerEnd + 1);
            }
