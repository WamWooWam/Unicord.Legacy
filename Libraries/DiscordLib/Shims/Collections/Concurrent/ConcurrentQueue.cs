...
namespace System.Collections.Concurrent
{
    ...
    public class ConcurrentQueue<T> : IProducerConsumerCollection<T>, IReadOnlyCollection<T>
    {
        ...
        /// returns false.</value>
        bool ICollection.IsSynchronized {get {return false;}} // always false, as true implies synchronization via SyncRoot
        ...
        public bool IsEmpty
        {
            get
            {
                ...
                T ignoredResult;
                return !TryPeek(out ignoredResult, false);
            }
        }
        ...
        public int Count
        {
            get
            {
                ...
                while (true)
                {
                    ...
                    head = _head;
                    tail = _tail;
                    headHead = head._headAndTail.Head;
                    headTail = head._headAndTail.Tail;
                    ...
                    else if (head._nextSegment == tail)
                    {
                        ...
                        tailHead = tail._headAndTail.Head;
                        tailTail = tail._headAndTail.Tail;
                        if (head == _head &&
                            tail == _tail &&
                            headHead == head._headAndTail.Head &&
                            headTail == head._headAndTail.Tail &&
        ...
        private static long GetCount(Segment head, int headHead, Segment tail, int tailTail)
        {
            ...
            int headTail = (head == tail ? tailTail : head._headAndTail.Tail) - head.FreezeOffset;
        ...
        public void CopyTo(T[] array, int index)
        {
            ...
            if (index < 0)
            {
                throw new ArgumentOutOfRangeException("index", "SR.Collection_CopyTo_ArgumentOutOfRangeException");
            }
        ...
        private T GetItemWhenAvailable(Segment segment, int i)
        {
            ...
            {
                var spinner = new SpinWait();
                while ((segment._slots[i].SequenceNumber & segment._slotsMask) != expectedSequenceNumberAndMask)
                {
                    spinner.SpinOnce();
                }
            }
        ...
        { 
            return (_head.TryDequeue(out result) || // fast-path that operates just on the head segment
        ...
        private sealed class Segment
        {
            ...
            internal volatile bool _preservedForObservation;
            ...
            internal volatile Segment _nextSegment;
            ...
            {
                // Validate the length
                Debug.Assert(boundedLength >= 2, string.Format("Must be >= 2, got {0}", boundedLength));
                Debug.Assert((boundedLength & (boundedLength - 1)) == 0, string.Format("Must be a power of 2, got {0}",boundedLength));
            ...
            internal int FreezeOffset {get{ return _slots.Length * 2;}}
            ...
            public bool TryDequeue(out T item)
            {
                ...
                while (true)
                {
                    ...
                    int sequenceNumber = _slots[slotsIndex].SequenceNumber;
                    ...
                    if (diff == 0)
                    {
                        ...
                        if (Interlocked.CompareExchange(ref _headAndTail.Head, currentHead + 1, currentHead) == currentHead)
                        {
                            ...
                            if (!_preservedForObservation)
                            {
                                ...
                                _slots[slotsIndex].SequenceNumber = currentHead + _slots.Length;
                            }
                            return true;
                        }
                    }
                ...
                while (true)
                {
                    ...
                    if (diff == 0)
                    {
                        ...
                        if (Interlocked.CompareExchange(ref _headAndTail.Tail, currentTail + 1, currentTail) == currentTail)
                        {
                            ...
                            _slots[slotsIndex].SequenceNumber = currentTail + 1;
                            return true;
                        }
                    }
                    else if (diff < 0)
    ...
    {
        [FieldOffset(128)] public volatile int Head;
        [FieldOffset(256)] public volatile int Tail;
    }
}